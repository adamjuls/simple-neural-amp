...
#endif
{
    // Load embedded JSON (added via juce_add_binary_data)
    const char* jsonPtr = (const char*) BinaryData::rtneural_model_lstm_32_json;
    const size_t jsonSize = (size_t) BinaryData::rtneural_model_lstm_32_jsonSize;
    std::string jsonString(jsonPtr, jsonPtr + jsonSize);
    std::istringstream jsonStream(jsonString);
    setupModel(lstmModel, jsonString);
}
...
void TestPluginAudioProcessor::setupModel(RTLSTMModel32& model, std::string jsonString){
    int lstm_units = 32; 
    std::istringstream jsonStream(jsonString);
